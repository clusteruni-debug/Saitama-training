...
> 글로벌 규칙 요약: 성급한 진단 금지(원인 3개 나열→검증→수정) | 구현 후 검증 필수(빌드/실행 확인 전 성공 보고 금지) | git push 전 git status 필수 | 민감 데이터 하드코딩 금지 | 요청 scope 전부 처리 | 작업 단위 완료 후 /clear 안내 | 실수 시 LEARNED 기록
